...
ACCESS_TOKEN_EXPIRE_MINUTES = int(os.getenv("ACCESS_TOKEN_EXPIRE_MINUTES")) * 24 * 7
...
def hash_password(password: str):
    password = hashlib.sha256(
        password.encode()
    ).hexdigest()  # pre-hash to avoid bcrypt 72-byte limit
...
def verify_password(plain: str, hashed: str) -> bool:
    ...
    return pwd_context.verify(
        password, hashed
    )  # if the password the user typed and the one that got hashed are == then return True
